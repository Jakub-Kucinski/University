...
async function get_user_backet(user_id) {
  try {
    var res = await pool.query("SELECT * FROM Basket WHERE Basket.user_id = $1", [user_id]);
    if (res.rowCount === 0) {
      return null;
    } else {
      return res.rows;
...
async function get_user_backet_describe(user_id) {
  try {
    var res = await pool.query("SELECT Product.name, Product.price, Product.link \
    FROM Basket INNER JOIN Product ON Basket.product_id = Product.product_id \
    WHERE Basket.user_id = $1", [user_id]);
    if (res.rowCount === 0) {
      return null;
    } else {
      return res.rows;
...
async function get_user_orders_describe(user_id) {
  try {
    var res = await pool.query("SELECT Product.name, Ordered.price, Product.link, Ordered.date_of_order, Ordered.amount \
    FROM Ordered INNER JOIN Product on Ordered.product_id = Product.product_id \
    WHERE Ordered.user_id = $1", [user_id]);
    if (res.rowCount === 0) {
      return null;
    } else {
...
async function get_backets_lists_describe_grouped_by_users() {
  try {
    ...
    for (var i = 0; i < number; i++){
      let user = users[i];
      let basket = await get_user_backet_describe(user.user_id);
      t.push({ user_id: user.user_id, login: user.login, basket: basket });
    }
    if (t.length === 0) {
      return null;
    } else {
...
async function get_orders_lists_describe_grouped_by_users() {
  try {
    var users = await users_list();
    var number = users.length;
    var t = [];
    for (var i = 0; i < number; i++){
      let user = users[i];
      let orders = await get_user_orders_describe(user.user_id);
      t.push({ user_id: user.user_id, login: user.login, orders: orders });
    }
    if (t.length === 0) {
      return null;
    } else {
...
async function main() {
  ...
  // console.log(wynik35);
  // wynik355 = await log_in_admin("admin", "admin");
  // console.log(wynik355);
...
module.exports = {
  ...
  remove_product_from_basket, make_order, user_orders_list, users_list, orders_list, baskets_list, get_user_backet_describe, 
  get_user_orders_describe, get_backets_lists_describe_grouped_by_users, get_orders_lists_describe_grouped_by_users
};