...
(define (pom xc)
    (if (null? (cdr xc))
        (mcons (car xc) null)
        (mcons (car xc) (pom (cdr xc)))))
(define (cycle xc head)
    (if (null? (mcdr xc))
        (set-mcdr! xc head)
        (cycle (mcdr xc) head)))

(define (make_cycle xs)
  (define (pom xc)
    (if (null? (cdr xc))
        (mcons (car xc) null)
        (mcons (car xc) (pom (cdr xc)))))
...
(define koniun (op '&& (op '> (const 2) (const 3)) (op '/ (const 1) (const 0))))
...
(define (interp p m)
  (match p
    ...
    [(if-stm b t e) (if (eval b m)
                        ...
                        (interp e m))]
    [(++ v)
     (env-update v (+ 1 (env-lookup v m)) m)]
    [(for x e1 e2 s1 s2)
     (interp
          (var-block x e1
                 (while e2
...
;[(for x e1 e2 s1 s2)
 ; (define (loop m2)
  ;  (if (eval e2 m2)
   ;   (loop (interp s1 (interp s2 m2)))
    ;  m2))
 ;(loop (env-add x (eval e1 m) m))]
...
(define fact2
  (var-block 'x (const 0)                                           
     ...
     (comp (for 'n (const 1)
             (op '<= (variable 'n) (variable 'i))
             (++ 'n)
             (assign 'x (op '* (variable 'x) (variable 'n))))
           (assign 'i (variable 'x))))))  
...
(define (makro-queen-inside-if n)
  (define (makro-!= k)
    (if (= k 1)
        (op '!= (gen-symbol 'y n) (gen-symbol 'y k))
        (op '&& (op '!= (gen-symbol 'y n) (gen-symbol 'y k)) (makro-!= (- k 1)))))
  (define (makro-abs k)
    (if (= k 1)
        (op '!= (abs (op '- (gen-symbol 'y n) (gen-symbol 'y k))) (op '- n k))
        (op '&& (op '!= (abs (op '- (gen-symbol 'y n) (gen-symbol 'y k))) (op '- n k)
                    (makro-!= (- k 1))))))
  (op '&& (makro-!= (- n 1)) (makro-abs (- n 1))))
