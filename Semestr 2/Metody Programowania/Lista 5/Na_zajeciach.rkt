...
(define (nnf? f)
  (or (var? f)
      ...
      (and (conj? f) (nnf? (conj-left f)) (nnf? (conj-right f)))
      (and (disj? f) (nnf? (disj-left f)) (nnf? (disj-right f)))))
...
(define (eval-formula f values)
  (cond [(var? f) (if (eq? #f (member (list f #t) values))
                      (if (eq? #f (member (list f #f) values))
                          (raise "blad")
                          #f)
                      #t)]
        [(lit? f) (let* ([x (lit-var f)]
                        [x-val (if (eq? #f (member (list x #t) values))
                                (if (eq? #f (member (list x #f) values))
                                    (raise "blad")
                                    #f)
                                #t)])
                    (if (lit-pol f)
...
(define (convert-to-cnf f)
  (define (convert f)
    (cond [(var? f) f]
          [(neg? f) f]
          [(disj? f) (let ([l (convert (disj-left f))]
                           [r (convert (disj-right f))])
                       (if (conj? l)
                           (if (conj? r)
                               (make-conj (make-conj (make-disj (conj-left l)
                                                                (conj-left r))
                                                     (make-disj (conj-left l)
                                                                (conj-right r)))
                                          (make-conj (make-disj (conj-right l)
                                                                (conj-left r))
                                                     (make-disj (conj-right l)
                                                                (conj-right r))))
                               (make-conj (make-disj (conj-left l) r)
                                          (make-disj (conj-right l) r)))
                           (if (conj? r)
                               (make-conj (make-disj l (conj-left r))
                                          (make-disj l (conj-right r)))
                               (make-disj l r))))]
          [(conj? f) (make-conj (convert (conj-left f)) (convert (conj-right f)))]))
